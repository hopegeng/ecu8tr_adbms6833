...
void Adbms6830_SetDefaultCommands(Adbms6830_CommandSet_t *cmds)
{
    ...
    /*
     * PLACEHOLDERS ONLY.
     * Replace with the exact opcodes from your approved documentation.
     */
    cmds->WRCFGA = 0x0001U;
    cmds->WRCFGB = 0x0024U;
    cmds->RDCFGA = 0x0002U;
    cmds->RDCVALL = 0x0004U;
    cmds->ADCV = 0x0260U;
    cmds->MUTE = 0x0028U;
    cmds->UNMUTE = 0x0029U;
}
                                                 ...
                                                 const Adbms6830_CommandSet_t *cmds)
{
    ...
    for (ic = (int32_t)ctx->icCount - 1; ic >= 0; ic--)
    {
        /* TODO: validate returned data PEC according to your official protocol. */
                                  ...
                                  uint32_t conversionDelayMs)
{
    ...
    switch (ctx->svcState)
    {
        ...
        case ADBMS6830_SVC_CONFIGURE:
            if ((Adbms6830_WriteCfga(ctx, hal, cmds) == ADBMS6830_OK) &&
                (Adbms6830_WriteCfgb(ctx, hal, cmds) == ADBMS6830_OK))
            {
                ctx->configured = true;
...
void Adbms6830_CfgbPackDccPlaceholder(uint16_t dccMask, uint8_t cfgb[ADBMS6830_BYTES_PER_CFGR])
{
    ...
    memset(cfgb, 0, ADBMS6830_BYTES_PER_CFGR);

    /* Placeholder only. Replace with exact CFGB bit map from the official docs. */
    cfgb[0] = (uint8_t)(dccMask & 0x00FFU);
    cfgb[1] = (uint8_t)((dccMask >> 8U) & 0x00FFU);
}
