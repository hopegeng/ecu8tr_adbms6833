 ...
 *	SLS_CAN		:	P15.2  	QSPI2_SLSO0	[O3]
 *	MOSI_CAN	:	P15.6 	QSPI2_MTSR	[O3]
 *	MISO_CAN	:	P15.7 	QSPI2_MRSTB	[I]
 *	SCK_CAN		:	P15.3 	QSPI2_SCLK	[O3]
 *
...
/*----- Macros ---------------------------------------------------------------*/
#define QSPI2_DMA_TX_CHANNEL 	(IfxDma_ChannelId_1)
#define QSPI2_DMA_RX_CHANNEL 	(IfxDma_ChannelId_2)


/*----- Prototypes -----------------------------------------------------------*/
#define QSPI2MASTER_DEBUG_PRINT 1

#if (QSPI2MASTER_DEBUG_PRINT == 1) && (__RL_SHELL == 1)
#define QSPI2M_DBG_WRITE(...) PRINTF("%sQSPI2 ", shell_GetCpu()); PRINTF(__VA_ARGS__)
#else
#define QSPI2M_DBG_WRITE(...)
#endif /* (QSPI2MASTER_DEBUG_PRINT == 1) && (__RL_SHELL == 1) */

#define kuQSPIMASTER_SLS_CS_ACTIVE_LOW									FALSE
#define kuQSPIMASTER_SLS_CS_ACTIVE_HIGH									TRUE

/* Number of CS */
#define kuQSPIMASTER_QSPI2_NUM_SLAVES									(1u)


/*----- Static member variables ----------------------------------------------*/
/**< \brief event semaphore for synchronization */
__private0 __far volatile uint32_t qspi2mstrEvent __align(4) = (0u);

/**< \brief Slave (CS) control structure */
__private0 __far QSPI_Slave_Ctl_t qspi2Slaves[kuQSPIMASTER_QSPI2_NUM_SLAVES] __align(4) =
{
	{
		/* SLSO0, P15.2 (OUTPUT) Active LOW (IfxQspi2_SLSO0_P15_2_OUT) */
		.port.lpPort = &MODULE_P15,
		.port.u8Pin = (2u),
		.u16Cs = eQspiHwCs00,	/* LTC6820 slave select is SLSO0 (P15.2) */
		 ...
		 * fPER = 200 (see Note1)
		 * GLOBALCON.TQ = 9 (set in qspi2mstr_Init())
...
boolean bIsQSPI2Init = FALSE;

/**< \brief QSPI2 configuration */
QSPIMSTR_Cfg_t qspi2mstrCfg __align(4) =
{
	.lpQspiModule = &MODULE_QSPI2,

	.pisel_mris = (0xb - 0xa), /* MRST input line B */

	/* Slave Select Control Array */
	.u16SlaveListSize = kuQSPIMASTER_QSPI2_NUM_SLAVES,
	.lpSlaveList = qspi2Slaves,
	.active_slave = eQspiHwCs00,

	/* QSPI SRC controls
	 * NB: QSPI tx/rx SRCs use DMA channel priorities and type of service.
	 */
	.srcrTxd.pSRC_SRCR = ((Ifx_SRC_SRCR*)&MODULE_SRC.QSPI.QSPI[2].TX),
	.srcrTxd.u16Srpn = ((uint16_t)QSPI2_DMA_TX_CHANNEL),
	.srcrTxd.tos = IfxSrc_Tos_dma,
	.srcrRxd.pSRC_SRCR = ((Ifx_SRC_SRCR*)&MODULE_SRC.QSPI.QSPI[2].RX),
	.srcrRxd.u16Srpn = ((uint16_t)QSPI2_DMA_RX_CHANNEL),
	.srcrRxd.tos = IfxSrc_Tos_dma,
	.srcrErr.pSRC_SRCR = ((Ifx_SRC_SRCR*)&MODULE_SRC.QSPI.QSPI[2].ERR),
	.srcrErr.u16Srpn = ((uint16_t)QSPI2_ERR_ISR_PRIORITY),
	.srcrErr.tos = QSPI2_CPU_TOS,

	/* DMA SRC controls
	 * NB: DMA tx/rx SRCs use CPU priorities and type of service.
	 */
	.srcrDmaTxCh.pSRC_SRCR = ((Ifx_SRC_SRCR*)&MODULE_SRC.DMA.DMA[0].CH[QSPI2_DMA_TX_CHANNEL]),
	.srcrDmaTxCh.u16Srpn = ((uint16_t)QSPI2_TxD_ISR_PRIORITY),
	.srcrDmaTxCh.tos = QSPI2_CPU_TOS,
	.srcrDmaRxCh.pSRC_SRCR = ((Ifx_SRC_SRCR*)&MODULE_SRC.DMA.DMA[0].CH[QSPI2_DMA_RX_CHANNEL]),
	.srcrDmaRxCh.u16Srpn = ((uint16_t)QSPI2_RxD_ISR_PRIORITY),
	.srcrDmaRxCh.tos = QSPI2_CPU_TOS,
 ...
 *
 ******************************************************************************/
IFX_INTERRUPT(qspi2DmaTxISR, QSPI2_CPU_VEC_TBL, QSPI2_TxD_ISR_PRIORITY)
{
    Ifx_DMA* pDma = &MODULE_DMA;


    /* Test if DMA tx channel interrupt is set */
    if (pDma->CH[QSPI2_DMA_TX_CHANNEL].CHCSR.B.ICH != 0)
    {
    	QSPIMSTR_Cfg_t* lpCntrl = &qspi2mstrCfg;
        ...
        pDma->CH[QSPI2_DMA_TX_CHANNEL].CHCSR.B.CICH = 1;
        /* Disable DMA tx channel transaction */
        pDma->TSR[QSPI2_DMA_TX_CHANNEL].B.DCH = 1;
 ...
 *
 ******************************************************************************/
IFX_INTERRUPT(qspi2DmaRxISR, QSPI2_CPU_VEC_TBL, QSPI2_RxD_ISR_PRIORITY)
{
    Ifx_DMA* pDma = &MODULE_DMA;


    /* Test if DMA rx channel interrupt is set */
    if (pDma->CH[QSPI2_DMA_RX_CHANNEL].CHCSR.B.ICH != 0)
    {
    	QSPIMSTR_Cfg_t* lpCntrl = &qspi2mstrCfg;
    	QSPI_Slave_Ctl_t* pSlave  = &lpCntrl->lpSlaveList[lpCntrl->active_slave];
    	...
    	pDma->CH[QSPI2_DMA_RX_CHANNEL].CHCSR.B.CICH = 1;
		...
		sem_Set(&qspi2mstrEvent, kuQSPI2_RX_EVENT);
    }
}
 ...
 *
 ******************************************************************************/
IFX_INTERRUPT(qspi2ErISR, QSPI2_CPU_VEC_TBL, QSPI2_ERR_ISR_PRIORITY)
{
	Ifx_DMA*			pDma	= &MODULE_DMA;
	QSPIMSTR_Cfg_t* 	lpCntrl	= &qspi2mstrCfg;
    QSPI_Slave_Ctl_t* 	pSlave	= &lpCntrl->lpSlaveList[lpCntrl->active_slave];
	...
	sem_Set(&qspi2mstrEvent, kuQSPI2_ERR_EVENT);
 ...
 *
 ******************************************************************************/
inline int32_t qspi2mstr_Init(boolean bState)
{
	QSPIMSTR_Cfg_t* lpCntrl = &qspi2mstrCfg;
	int32_t i32Rtn = -1;
	...
	if((bState == TRUE) && (bIsQSPI2Init == FALSE))
	{/* Request to ENABLE the QSPI module */
		...
		pDma->TSR[QSPI2_DMA_TX_CHANNEL].U = 0x00020000;
		pDma->TSR[QSPI2_DMA_RX_CHANNEL].U = 0x00020000;

		/* Initialize DMA tx channel */
		pDma->CH[QSPI2_DMA_TX_CHANNEL].CHCFGR.U = lpCntrl->txDmaChannelCfg.U;
		pDma->CH[QSPI2_DMA_TX_CHANNEL].ADICR.U = lpCntrl->txDmaAddrIntCtl.U;

		/* Initialize DMA rx channel */
		pDma->CH[QSPI2_DMA_RX_CHANNEL].CHCFGR.U = lpCntrl->rxDmaChannelCfg.U;
		pDma->CH[QSPI2_DMA_RX_CHANNEL].ADICR.U = lpCntrl->rxDmaAddrIntCtl.U;
	...
	else if((bState == FALSE) && (bIsQSPI2Init == TRUE))
	{/* Request to DISABLE the QSPI module */
		...
		pDma->CH[QSPI2_DMA_TX_CHANNEL].ADICR.U = 0u;
		pDma->CH[QSPI2_DMA_RX_CHANNEL].ADICR.U = 0u;
 ...
 *
 * Completion of the operation is signaled by a qspi2mstrEvent bit set in the
 * DMA rx ISR.
 *
 ...
 *
 ******************************************************************************/
int32_t qspi2mstr_Send(QspiCs_t CS, uint16_t u16Length, uint8_t* pu8SrcBuff,
						uint8_t* pu8DstBuff)
{
	int32_t i32Rtn = 0;

	if((bIsQSPI2Init == TRUE) && (pu8SrcBuff != NULL) &&
		(pu8DstBuff != NULL) && (u16Length > 0) && (CS == 0))
	{
        Ifx_DMA				*pDma         	= &MODULE_DMA;
		QSPIMSTR_Cfg_t*		lpCntrl			= &qspi2mstrCfg;
		...
		if (u16Length > 1)
		{
			 ...
			 */
			pDma->CH[QSPI2_DMA_TX_CHANNEL].CHCFGR.B.TREL = pSlave->tx.remaining - 1;
			/* Source of tx channel is tx buffer in slave control structure */
			pDma->CH[QSPI2_DMA_TX_CHANNEL].SADR.U = (uint32)((void *)IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), pSlave->tx.data));
			/* Destination of tx channel is QSPI tx FIFO */
			pDma->CH[QSPI2_DMA_TX_CHANNEL].DADR.U = (uint32)&lpCntrl->lpQspiModule->DATAENTRY[CS].U;
		}
		else
		{
		 ...
		 */
        pDma->CH[QSPI2_DMA_RX_CHANNEL].CHCFGR.B.TREL = u16Length;
        /* Source of rx channel is QSPI rx FIFO */
        pDma->CH[QSPI2_DMA_RX_CHANNEL].SADR.U =(uint32)&lpCntrl->lpQspiModule->RXEXIT.U;
        /* Destination of rx channel is rx buffer in slave control structure */
        pDma->CH[QSPI2_DMA_RX_CHANNEL].DADR.U = (uint32)((void *)IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), pSlave->rx.data));
        ...
        pDma->TSR[QSPI2_DMA_RX_CHANNEL].B.ECH = 1;
        ...
        if (u16Length > 1)
        {
        	...
        	pDma->TSR[QSPI2_DMA_TX_CHANNEL].B.ECH = 1;
 ...
 * @author  JW
 * @date    Jun2018 (created)
 * @brief   Function: qspi2mstr_Enable()
 *
...
{
	return(qspi2mstr_Init(bState));
}/* END qspi2mstr_Enable() */
 ...
 * @brief   Function: qspi2mstr_waitForRxDone()
...
boolean qspi2mstr_waitForRxDone( void )
{
	while( TRUE )
	{
		if( sem_TestForEvent( &qspi2mstrEvent, kuQSPI2_RX_EVENT) == TRUE )
		{
			return TRUE;
		}

		if( sem_TestForEvent( &qspi2mstrEvent, kuQSPI2_ERR_EVENT) == TRUE )
		{
			return FALSE;
		}
	}
} /* END of qspi2mstr_waitForRxDone */
